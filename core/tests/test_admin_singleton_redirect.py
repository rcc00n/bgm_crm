...
from core.models import AdminLoginBranding, ShopRateSettings, SiteHoursSettings
...
class AdminSingletonRedirectTests(TestCase):
    ...
    def test_site_hours_settings_changelist_redirects_to_change_form(self):
        settings_obj = SiteHoursSettings.objects.get(singleton_id=1)

        response = self.client.get(reverse("admin:core_sitehourssettings_changelist"), secure=True)

        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response.headers["Location"],
            reverse("admin:core_sitehourssettings_change", args=[settings_obj.pk]),
        )
